...
function linkedLabel(data: AppData, categoryId: string) {
  return (
    data.categories.find((item) => item.id === categoryId)?.name ?? "Kategori"
  );
}
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-6">
        ...
        <Card className="surface-elevated w-full overflow-hidden border-primary/15 bg-linear-to-br from-[#1a2b23] via-[#172520] to-[#223126] text-white">
          <CardContent className="space-y-6 px-6 pb-7 pt-10 md:px-7 md:pb-8 md:pt-11">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <p className="font-display text-4xl font-semibold tracking-[-0.06em]">
                    {balanceHidden
                      ? "••••••••••"
                      : formatCurrency(data.dashboardSummary.totalBalance)}
                  </p>
                  <button
                    type="button"
                    aria-label={
                      balanceHidden ? "Tampilkan saldo" : "Sembunyikan saldo"
                    }
            ...
            <div className="rounded-[22px] border border-white/8 bg-white/6 px-4 py-3">
              ...
              <p className="mt-2 text-sm leading-6 text-white/88">
                Pindahkan sebagian freelance income ke dana darurat untuk
                percepat target tahunan.
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        <MetricCard
          label="Total balance"
          value={data.dashboardSummary.totalBalance}
          trend={10.8}
          detail="Akumulasi semua wallet aktif."
        />
        <MetricCard
          label="Income bulan ini"
          value={data.dashboardSummary.monthlyIncome}
          trend={8.4}
          detail="Salary + freelance."
        />
        <MetricCard
          label="Expense bulan ini"
          value={data.dashboardSummary.monthlyExpense}
          trend={-2.1}
          detail="Termasuk transfer ke goal."
        />
        <MetricCard
          label="Asset value"
          value={data.dashboardSummary.assetValue}
          trend={6.9}
          detail={`Savings rate ${formatPercent(data.dashboardSummary.savingsRate)}`}
        />
      </div>
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <ChartCard
          title="Cashflow 5 bulan terakhir"
          description="Garis income dan expense ditata untuk membaca tren secara cepat."
          aside={<Badge variant="secondary">Live mock</Badge>}
        >
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <ChartCard
          title="Budget health"
          description="Kategori yang mendekati limit diberi sinyal visual yang jelas."
        >
          <BudgetBars data={data.budgetProgress} />
        </ChartCard>
        <ChartCard
          title="Goal progress"
          description="Setiap goal terhubung dengan wallet khusus atau alokasi transaksi."
        >
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <Card className="surface-card">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Terhubung dengan halaman transaksi, debt, asset, dan goal.
              </CardDescription>
            </div>
          ...
          <CardContent className="space-y-3">
            {data.transactions.slice(0, 5).map((transaction) => (
              <div
                key={transaction.id}
                className="flex items-center justify-between rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-3"
              >
                <div>
        ...
        <Card className="border-amber-500/20 bg-linear-to-b from-amber-500/8 to-transparent">
          <CardHeader>
            ...
            <CardDescription>
              Warning states yang paling penting selalu dekat dengan action
              area.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-[24px] border border-amber-500/20 bg-amber-500/10 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.18)]">
              ...
              <p className="text-sm leading-6 text-muted-foreground">
                Rp1.140.000 dari Rp1.200.000 sudah terpakai. Review sebelum auto
                debit berikutnya.
              </p>
            </div>
            {data.reminders.slice(0, 2).map((reminder) => (
              <div
                key={reminder.id}
                className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] p-4"
              >
                <div className="flex items-center justify-between gap-4">
                  ...
                  <Badge
                    variant={
                      reminder.status === "today" ? "warning" : "secondary"
                    }
                  >
                    {reminder.status}
                  </Badge>
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <FormDrawer
            title="Tambah wallet demo"
            description="Simulasi membuat wallet baru."
            triggerLabel="Tambah wallet"
          />
        }
      />
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
        {data.wallets.map((wallet) => (
          <Card
            key={wallet.id}
            className={wallet.type === "goal" ? "border-primary/35" : ""}
          >
            <CardContent className="space-y-4 p-5">
              <div className="flex items-center justify-between">
                <Badge
                  variant={wallet.type === "goal" ? "default" : "secondary"}
                >
                  {wallet.type}
                </Badge>
                <StatusBadge
                  status={wallet.changePercent > 0 ? "healthy" : "info"}
                />
              </div>
              <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {wallet.changePercent}% vs bulan lalu
              </p>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Mock flow untuk memindahkan saldo antar sumber dana.
              </CardDescription>
            </div>
            <FormDrawer
              title="Transfer antar-wallet"
              description="Simulasi transfer internal."
          ...
          <CardContent className="space-y-4">
            {[
              ...
              <div
                key={note}
                className="rounded-[22px] border border-border/70 bg-background/70 px-4 py-4 text-sm text-muted-foreground"
              >
                {note}
              </div>
            ))}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Aktivitas yang paling relevan untuk setiap sumber dana.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
    ...
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <FormDrawer
            ...
            description="Expense, income, atau transfer demo."
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="surface-elevated">
          <CardContent className="p-5">
            ...
            <p className="metric-figure mt-2">
              {formatCompactCurrency(data.dashboardSummary.monthlyIncome)}
            </p>
          </CardContent>
        </Card>
        <Card className="surface-elevated">
          <CardContent className="p-5">
            ...
            <p className="metric-figure mt-2">
              {formatCompactCurrency(data.dashboardSummary.monthlyExpense)}
            </p>
          </CardContent>
        </Card>
        <Card className="surface-elevated">
          <CardContent className="p-5">
            <p className="eyebrow">Linked flows</p>
            <p className="metric-figure mt-2">3</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Debt, asset, dan goal relation aktif.
            </p>
          </CardContent>
        </Card>
      </div>
      <Card className="surface-card">
        <CardHeader>
          ...
          <CardDescription>
            Searchable table dengan linked entity chips.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable
              ...
              },
              {
                ...
                render: (row) => (
                  <div className="flex flex-wrap gap-2">
                    ...
                    {!row.assetId && !row.debtId && !row.goalId ? (
    ...
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <FormDrawer
            title="Tambah budget"
            description="Set limit kategori bulanan."
            triggerLabel="Tambah budget"
          />
        }
      />
      <div className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
        <ChartCard
          ...
          description="Bar progress menonjolkan kategori paling rawan overspend."
        >
          <BudgetBars data={data.budgetProgress} />
        </ChartCard>
        <Card className="border-amber-500/20 bg-linear-to-b from-amber-500/8 to-transparent">
          <CardHeader>
            ...
            <CardDescription>
              AI-style recommendations dengan konteks yang mudah dipahami.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {[
              ...
              <div
                key={item}
                className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4 text-sm leading-6 text-muted-foreground"
              >
                {item}
              </div>
            ))}
            ...
            <CardContent className="space-y-3 p-5">
              <div className="flex items-center justify-between">
                <p className="font-medium">
                  {linkedLabel(data, budget.categoryId)}
                </p>
                <Badge
                  variant={
                    budget.spent / budget.amountLimit > 0.9
                      ? "warning"
                      : "success"
                  }
                >
                  {Math.round((budget.spent / budget.amountLimit) * 100)}%
                </Badge>
              </div>
    ...
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <FormDrawer
            title="Tambah asset"
            description="Catat investasi atau aset fisik secara manual."
            triggerLabel="Tambah asset"
          />
        }
      />
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <ChartCard
          ...
          description="Komposisi aset fisik dan digital."
        >
        ...
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Table sederhana untuk nilai saat ini dan cost basis.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {data.assets.map((asset) => (
              <div
                key={asset.id}
                className="rounded-[24px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4"
              >
                <div className="flex items-center justify-between gap-4">
                  <div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {data.debtOverview.map((item) => (
          <MetricCard
            ...
            label={item.label}
            value={item.value}
            detail="Terhubung dengan alur transaksi."
          />
        ))}
      </div>
                ...
                .map((debt) => (
                  <Card key={debt.id} className="surface-card">
                    <CardContent className="space-y-4 p-5">
                      ...
                      <Progress
                        value={(debt.paid / (debt.paid + debt.amount)) * 100}
                      />
                    </CardContent>
                  </Card>
                ))}
    ...
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <FormDrawer
            title="Tambah goal"
            description="Buat target finansial baru."
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <ChartCard
          title="Goal progress"
          description="Visual progress untuk target utama bulan ini."
        >
          <GoalTimeline data={data.goalProgress} />
        </ChartCard>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Hal kecil yang membantu pengguna tetap bergerak.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {[
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <ChartCard
          ...
          description="Perbandingan pemasukan dan pengeluaran per bulan."
        >
          <CashflowChart data={data.cashflowSeries} />
        </ChartCard>
        <ChartCard
          title="Allocation"
          description="Distribusi aset dan fokus portfolio saat ini."
        >
          <AllocationChart data={data.assetAllocation} />
        </ChartCard>
      </div>
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Metrix singkat untuk membaca kesehatan finansial.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2">
            <MetricCard
              label="Savings rate"
              value={31_000_000}
              detail="Disajikan sebagai KPI besar untuk emphasis visual."
            />
            <MetricCard
              label="Debt ratio"
              value={18_000_000}
              detail="Terkalkulasi dari total utang aktif terhadap asset + cash."
            />
          </CardContent>
        </Card>
      </div>
    ...
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <FormDrawer
            title="Buat automasi"
            description="Simulasi recurring transaction atau bill reminder."
            triggerLabel="Buat automasi"
          />
        }
      />
      <div className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Daftar tagihan berulang yang biasanya perlu dipantau per bulan.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {data.reminders.map((reminder) => (
              <div
                key={reminder.id}
                className="flex items-center justify-between rounded-[24px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4"
              >
                <div>
                  <p className="font-medium">{reminder.title}</p>
                  <p className="text-sm text-muted-foreground">
...
export function RemindersView({ data }: { data: AppData }) {
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <FormDrawer
            ...
            description="Buat pengingat baru untuk tagihan atau kebiasaan."
            triggerLabel="Tambah reminder"
          />
        }
      />
      <div className="grid gap-4 md:grid-cols-3">
      ...
      <EmptyState
        title="Reminders archive masih kosong"
        description="Area ini bisa dipakai untuk reminder yang sudah selesai, di-snooze, atau diarsipkan setelah backend tersedia."
        actionLabel="Buat reminder baru"
      />
    </div>
  );
}
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Seeded data dengan status unread, channel, dan message copy.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
        ...
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Contoh toggle settings yang konsisten dengan halaman Settings.
            </CardDescription>
          </CardHeader>
            ...
            {[
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Kontrol kanal notifikasi sesuai preferensi.
                  </p>
                </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Grouping per hari untuk memudahkan review device dan tindakan
              akun.
            </CardDescription>
          </CardHeader>
            ...
            {data.auditEventGroups.map((group) => (
              <div key={group.date} className="space-y-4">
                ...
                {group.items.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4"
                  >
                    <div className="flex items-center justify-between gap-4">
                      <div>
                      ...
                      <Badge
                        variant={
                          item.severity === "warning" ? "warning" : "secondary"
                        }
                      >
                        {item.severity}
                      </Badge>
        ...
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Area penguatan trust untuk fitur akun sensitif.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {[
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
            <Card className="surface-card">
              <CardHeader>
                ...
                <CardDescription>
                  Pengaturan dasar yang paling sering dipakai.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                <div>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Tema global dikendalikan dari toggle header dan preview di
                      sini.
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Ringkasan mingguan dan pemberitahuan akun.
                    </p>
                  </div>
                  <Switch
                    defaultChecked={data.currentUser.notificationSettings.email}
                  />
                </div>
              </CardContent>
            </Card>
            <Card className="surface-card">
              <CardHeader>
                ...
                <CardDescription>
                  Kontrol harian untuk kenyamanan penggunaan.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                {[
                  ...
                  <div
                    key={item}
                    className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4 text-sm text-muted-foreground"
                  >
                    {item}
                  </div>
                ))}
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
            <Card className="surface-card">
              <CardHeader>
                ...
                <CardDescription>
                  2FA, device checks, dan password routines.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Minta konfirmasi untuk perangkat baru.
                    </p>
                  </div>
                ...
                <div className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4 text-sm leading-6 text-muted-foreground">
                  Perubahan keamanan sensitif akan tercatat di AuditLog dan
                  memicu notifikasi akun.
                </div>
              </CardContent>
            </Card>
            <Card className="surface-card">
              <CardHeader>
                <CardTitle>Security center</CardTitle>
                <CardDescription>
                  Status trust dan aktivitas terakhir.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {[
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
            <Card className="surface-card">
              <CardHeader>
                ...
                <CardDescription>
                  Status plan premium dan sisa kuota fitur pintar.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                <div className="rounded-[24px] border border-primary/20 bg-primary/8 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.18)]">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Unlimited wallets, advanced analytics, AI assistant.
                  </p>
                </div>
                <div className="space-y-2">
            ...
            <Card className="surface-card">
              <CardHeader>
                ...
                <CardDescription>
                  Benefit utama paket yang sedang aktif.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {data.subscriptionPlans
                  .find((plan) => plan.id === "premium")
                  ?.features.map((feature) => (
                    <div
                      key={feature}
                      className="flex items-center gap-3 rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4 text-sm"
                    >
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
            <Card className="surface-card">
              <CardHeader>
                ...
                <CardDescription>
                  Ringkasan produk dan positioning.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 text-sm text-muted-foreground">
                <p>
                  fyntra. adalah dashboard finansial pribadi untuk budgeting,
                  asset monitoring, goal tracking, dan security visibility.
                </p>
                <div className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4">
                  Faizax FinanceOS dirancang untuk membuat cashflow, asset,
                  debt, dan goal lebih mudah dibaca dalam satu workspace.
                </div>
              </CardContent>
            </Card>
            <Card className="surface-card">
              <CardHeader>
                ...
                <CardDescription>
                  Konten bantuan kini disatukan ke dalam settings.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-4">
                {[
                  ...
                  <div
                    key={item}
                    className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4 text-sm"
                  >
                    <p className="font-medium text-foreground">{item}</p>
                    <p className="mt-2 text-muted-foreground">
                      Konten bantuan lengkap dapat dipasang nanti tanpa mengubah
                      struktur settings ini.
                    </p>
                  </div>
                ))}
              </CardContent>
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
            <Card className="surface-card">
              <CardHeader>
                <CardTitle>Notification feed</CardTitle>
                <CardDescription>
                  Notifikasi penting dan reminder kini berada dalam satu
                  section.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
            ...
            <div className="space-y-6">
              <Card className="surface-card">
                <CardHeader>
                  <CardTitle>Channel preferences</CardTitle>
                  <CardDescription>
                    Kontrol kanal notifikasi dan reminder.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-5">
                  {[
              ...
              <Card className="surface-card">
                <CardHeader>
                  ...
                  <CardDescription>
                    Reminder tagihan dan habit dipindah ke section ini.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {data.reminders.map((reminder) => (
                    <div
                      key={reminder.id}
                      className="flex items-center justify-between rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4"
                    >
                      <div>
                        <p className="font-medium">{reminder.title}</p>
                        <p className="text-sm text-muted-foreground">
          ...
          <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <Card className="surface-card">
              <CardHeader>
                ...
                <CardDescription>
                  Ringkasan cepat aktivitas sensitif akun.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4 text-sm text-muted-foreground">
                  Aktivitas login, perubahan transaksi, dan verifikasi device
                  tercatat dalam satu feed yang mudah diaudit.
                </div>
                <div className="rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4 text-sm text-muted-foreground">
                  Semua perubahan sensitif diposisikan dekat dengan section
                  keamanan agar lebih mudah ditinjau.
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {data.subscriptionPlans.map((plan) => (
          <Card
            ...
            className={
              plan.id === "premium"
                ? "surface-elevated border-primary/20"
                : "surface-card"
            }
          >
  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        action={
          <Button asChild>
            <Link href="/app/dashboard">Lewati ke dashboard</Link>
          </Button>
        }
      />
      <div className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Step-by-step yang ringan untuk pengguna baru.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {[
              ...
              <div
                key={item.label}
                className="flex items-center gap-3 rounded-[22px] border border-border/70 bg-[color:var(--surface-2)] px-4 py-4"
              >
                <div
                  className={`flex size-10 items-center justify-center rounded-full ${item.done ? "bg-primary/10 text-primary" : "bg-secondary text-secondary-foreground"}`}
                >
                  {item.done ? (
        ...
        <Card className="surface-card">
          <CardHeader>
            ...
            <CardDescription>
              Data awal yang sudah disiapkan agar user langsung melihat value
              product.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            <div className="rounded-[24px] border border-border/70 px-5 py-4">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {data.wallets[0]?.name} dengan saldo{" "}
                {formatCurrency(data.wallets[0]?.balance ?? 0)}
              </p>
            </div>
            <div className="rounded-[24px] border border-border/70 px-5 py-4">
              <p className="font-medium">Goal pertama</p>
              <p className="mt-1 text-sm text-muted-foreground">
                {data.goals[0]?.name} dengan target{" "}
                {formatCurrency(data.goals[0]?.targetAmount ?? 0)}
              </p>
            </div>
            <div className="rounded-[24px] border border-border/70 px-5 py-4">
              <p className="font-medium">Reminder starter pack</p>
              <p className="mt-1 text-sm text-muted-foreground">
                Bill reminder + habit reminder siap diaktifkan.
              </p>
            </div>
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <FormDrawer
                title="Edit onboarding setup"
                description="Simulasi pengisian wallet, goal, dan preferensi reminder."
                triggerLabel="Edit setup"
              />
            </div>
          </CardContent>
        </Card>
